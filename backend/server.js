...
const JWT_SECRET = 'your_jwt_secret_key'; // Поменяй на свой секрет
const TEAMS_DIR = path.join(__dirname, 'teams'); // Директория для хранения HTML-файлов команд
const TEAMS_FILE = path.join(__dirname, 'teams', 'teams.json');
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: 'Регистрация успешна. Пожалуйста, проверьте вашу почту для подтверждения регистрации.' });
  } catch (error) {
    console.error('Ошибка при отправке письма:', error);
    return res.status(500).json({ message: 'Ошибка при отправке письма подтверждения' });
  }
});
...
app.post('/api/teams', authMiddleware, teamUpload.fields([
  ...
  writeTeams(teams);
    ...
    const html = teamHtmlTemplate(newTeam);
    fs.writeFileSync(filePath, html, 'utf8');
